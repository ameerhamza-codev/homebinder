...
import 'features/home_features.dart';
...
class _DetailsState extends State<Details> {
  ...
  Future<List<HomeFeatureModel>> getFeatures(apiToken,email)async{
    print("token $apiToken : email $email");
            ...
            child: Column(
              children: [
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    const Text("Features",style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w500),),
                    IconButton(
                        onPressed: (){
                          Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) =>  HomeFeatures()));
                        },
                        icon: Icon(Icons.open_in_new))
                  ],
                ),
                ...
                Container(
                  height: 180,
                  child: FutureBuilder<List<HomeFeatureModel>>(
                      future: getFeatures(provider.userData!.authenticationToken,provider.userData!.email),
                      builder: (context,AsyncSnapshot<List<HomeFeatureModel>> snapshot) {
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          return Center(
                            child: CircularProgressIndicator(),
                          );
                                      ...
                                      InkWell(
                                        ...
                                        child: Column(
                                          children: [
                                            ...
                                            SizedBox(width: 100,child: Text(snapshot.data![index].name!,maxLines: 3,))
                                          ],
                                        ),
                                      ),
                                      const SizedBox(width: 10,),
